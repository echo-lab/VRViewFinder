...
public class OptitrackStreamingClient : MonoBehaviour
{
	...
	/// ZACH ADDED
	public void Initialize(string _LocalAddress, string _ServerAddress, int _ServerCommandPort, int _ServerDataPort){
		LocalAddress = _LocalAddress;
		ServerAddress = _ServerAddress;
		ServerCommandPort = (UInt16)_ServerCommandPort;
		ServerDataPort = (UInt16)_ServerDataPort;
		OnEnable ();
	}
    ...
    private void OnNatNetFrameReceived( object sender, NatNetClient.NativeFrameReceivedEventArgs eventArgs )
    {
        ...
        try
        {
			...
			//Debug.Log("frame RB count: " + frameRbCount);
            ...
            for ( int rbIdx = 0; rbIdx < frameRbCount; ++rbIdx )
            {
				...
				//Debug.Log("rbState.Pose.Position: " + rbState.Pose.Position);
