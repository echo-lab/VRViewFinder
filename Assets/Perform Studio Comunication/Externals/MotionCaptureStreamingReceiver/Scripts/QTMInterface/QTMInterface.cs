...
public class QTMInterface : MonoBehaviour {
    ...
    */
	...
	public void Initialize(string _serverIP, int _serverPort, int _localPort) {
		serverIP = _serverIP;
		serverPort = _serverPort;
		localPort = _localPort;
    ...
    private void ExtractPositionsFrom3DNoLabels(OscMessage message, ref List<Vector3> positions) {
        int numMarkers = (int)message.Data[0];
		//Debug.Log ("# of incoming unidentified markers: " + numMarkers);
        int indexCounter = 0;
        for (int i = 0; i < numMarkers; i++) {
            Vector3 pos;
            int markerNum = (int)message.Data[indexCounter + 4]; // unused
            pos.x = (float)message.Data[indexCounter + 1] * .001f;
			pos.y = (float)message.Data[indexCounter + 2] * .001f;
			pos.z = (float)message.Data[indexCounter + 3] * .001f;
    ...
    private void ExtractPositionAndRotationFrom6D(OscMessage message, ref Vector3 position, ref Quaternion rotation) {
